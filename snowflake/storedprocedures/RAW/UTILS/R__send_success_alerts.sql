...
DECLARE
    ...
    v_duration FLOAT;
    v_summary_msg VARCHAR;
    v_email_subject VARCHAR;
    v_email_body VARCHAR;
BEGIN
    -- 1. Fetch job details from the LOG_HISTORY table
    SELECT SP_NAME, COALESCE(ROWS_PROCESSED, 0), COALESCE(DURATION_SECONDS, 0), COALESCE(MESSAGE, ''Completed successfully.'')
    INTO :v_sp_name, :v_rows_processed, :v_duration, :v_summary_msg
    FROM {{ databases.RAW }}.UTILS.LOG_HISTORY
    WHERE JOB_ID = :JOB_ID;

    -- 2. Construct the email components
    v_email_subject := ''✅ MDC Alert: '' || :v_sp_name || '' SUCCESS'';
    
    -- Added margin: auto; to center the email body
    v_email_body := ''
    <div style="font-family: Arial, sans-serif; border: 1px solid #ddd; border-radius: 8px; overflow: hidden; max-width: 600px; margin: auto;">
        <div style="background-color: #28a745; padding: 15px; color: white; text-align: center;">
            <h2 style="margin: 0;">Procedure Execution Successful</h2>
        </div>
        <div style="padding: 20px; color: #333;">
            ...
            <p style="background: #f8f9fa; padding: 10px; border-radius: 4px; font-family: monospace; font-size: 12px; color: #333;">'' 
            || :v_summary_msg || ''
            </p>
        </div>
    </div>'';
